...
export default function AuthModal({ isOpen, onClose }) {
  ...
  const onSubmit = (data) => {
    ...
    const ADMIN_EMAIL = "admin@luvia.com";
    const ADMIN_PASS = "admin123";

    if (isLogin) {
      if (data.email === ADMIN_EMAIL && data.password === ADMIN_PASS) {
        const adminUser = { ...data, name: "Head of Luvia", isAdmin: true };
        dispatch({ type: "LOGIN", payload: adminUser });
        toast.success("Admin Access Granted");
        onClose();
        navigate("/admin");
      } else {
        const userMatch = savedUsers.find(
      ...
      if (userExists) {
        ...
        const newUser = { ...data, id: Date.now(), isAdmin: false };
      ...
      {isOpen && (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-6">
          ...
          <motion.div
            ...
            className="relative w-full max-w-md bg-white dark:bg-zinc-900 p-10 rounded-[40px] shadow-2xl overflow-hidden text-brand-berry dark:text-brand-nude"
          >
            <button
              onClick={onClose}
              className="absolute top-6 right-6 text-gray-400 hover:text-brand-berry transition"
            >
              <X size={20} />
            </button>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-brand-berry">
              {!isLogin && (
                <input
                  {...register("name", { required: true })}
                  placeholder="Full Name"
                  className="w-full p-4 rounded-2xl bg-brand-nude dark:bg-luxury-black border-none focus:ring-2 focus:ring-brand-rose/50 outline-none dark:text-brand-nude"
                />
              )}
              <input
